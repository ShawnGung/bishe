...
def getUrl(facility):
    ...
    for oneCity in dList:
        ...
        for oneDistrict in dList[oneCity] :
            ...
            results,total = waitRequest(url,results,oneCity)
            #total_page = total // 20
            total_page = 5
            print(oneDistrict)
            ...
            for i in range(0,total_page):
                url = 'http://api.map.baidu.com/place/v2/search?' + params + '&output=json&ak='+ ak +'&page_size=20&page_num=' + str(i+1)
                results,total = waitRequest(url,results,oneCity)
                print(id(results))
...
def waitRequest(url,results,oneCity):
    for waitCount in range(0,5):
        ...
        if dictData['message'] == 'ok':
            ...
            time.sleep(5)
    return results,0 #固定抓取5页
    #return results,dictData['total']
...
def saveInCsv(results,facility):
    for each in results:
        for pos in results[each]:
            ...
            dataframe.to_csv(each + '-' + facility + '.csv',index=False)
